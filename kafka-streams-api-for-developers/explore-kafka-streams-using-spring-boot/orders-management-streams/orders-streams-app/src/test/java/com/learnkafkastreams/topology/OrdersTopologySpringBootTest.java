...
class OrdersTopologySpringBootTest {
    ...
    private static void assertWindowedOrderRevenue(KeyValue<Windowed<String>, TotalRevenue> keyValue, OrderType orderType) {
        ...
        if (OrderType.GENERAL.name().equals(orderType.name())) {
            assertEquals(0, new BigDecimal("54.00").compareTo(totalRevenue.runningRevenue()));
        } else {
            assertEquals(0, new BigDecimal("30.00").compareTo(totalRevenue.runningRevenue()));
        }
